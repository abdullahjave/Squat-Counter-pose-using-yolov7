...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        # Check if file exists with .pt extension if not found
        weight_path = Path(w)
        if not weight_path.exists() and not str(w).endswith('.pt'):
            weight_path = Path(str(w) + '.pt')
            if weight_path.exists():
                w = str(weight_path)
                print(f'Using {w} instead of {weights}')
